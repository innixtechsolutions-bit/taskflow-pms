...
**Discovered dependency (found during implementation)**: T038/T038c's edit/delete
controls need actual work-item rows rendered in `project-detail` to attach
to, but per T034's scope note, the itemized list itself was deliberately
deferred to US6's T054-T057. Rather than build a throwaway partial list now
and rebuild it again for US6's filter/search/pagination pass, T054/T055 (the
listing endpoint) and a basic unfiltered rendering of it in `project-detail`
are built here, in US4, ahead of their nominal phase — keeping their task
IDs and `[US6]` labels unchanged so numbering stays stable. US6's own
T053/T056/T057 then *extend* this already-existing list with a filter/search
bar and pagination controls, rather than creating the list from scratch.

### Tests for User Story 4 ⚠️

- [X] T035 [P] [US4] Unit tests for `WorkItemService.UpdateAsync` in `backend/TaskFlow.Api.Tests/Services/WorkItemServiceTests.cs`: creator/current assignee/Manager/Admin can update any field including status and `UpdatedAt` advances; any other caller is rejected; nothing can change `ProjectId`. **Confirmed RED**: `WorkItemService` had no `UpdateAsync` (CS1061).
...
- [X] T037 [P] [US4] Vitest tests for edit mode in `frontend/src/app/projects/work-item-form/work-item-form.component.spec.ts`: pre-fills existing values (including each `<select>` correctly pre-selecting its current value), submits changes. Written together with its implementation in the same pass rather than confirmed RED separately; **confirmed GREEN**: 45/45 frontend tests (was 43, +2, 0 regressions).
- [X] T038 [P] [US4] Vitest tests added to `frontend/src/app/projects/project-detail/project-detail.component.spec.ts`: the **edit** control is shown for a work item's creator/current assignee/Manager/Admin and hidden for an unrelated viewer. **Confirmed RED**: `project-detail` fetched no work items and rendered no rows, so `#work-item-{id} .edit-link` never existed.
- [X] T038a [P] [US4] Unit tests for `WorkItemService.DeleteAsync` in `backend/TaskFlow.Api.Tests/Services/WorkItemServiceTests.cs`: creator/Manager/Admin can delete; the current assignee alone (not also creator/Manager/Admin) is rejected; an unrelated caller is rejected; an unknown id throws not-found
- [X] T038b [P] [US4] Integration tests for `DELETE /api/work-items/{id}` in `backend/TaskFlow.Api.Tests/Integration/WorkItemsEndpointsTests.cs`: `204` for creator/Manager/Admin, `403` for the assignee-alone case and for an unrelated caller, `404` for an unknown id
- [X] T038c [P] [US4] Vitest tests added to `frontend/src/app/projects/project-detail/project-detail.component.spec.ts`: the **delete** control is shown only for a work item's creator/Manager/Admin and hidden for the assignee-alone case and for an unrelated viewer — narrower than the edit control tested in T038. **Confirmed RED** alongside T038 (same missing-rows cause).
- [X] T038d [P] [US4] Integration test in `backend/TaskFlow.Api.Tests/Integration/AuthEndpointsTests.cs` asserting `POST /api/auth/login`'s `AuthResponse` and `GET /api/auth/me`'s `MeResponse` both include the caller's own `id`, matching the signed-in user's actual id — must be written and confirmed **RED** before T039 is implemented (mirrors T024/T026 preceding T030 in Feature 001). **Confirmed RED**: `AuthResponse`/`MeResponse` didn't have an `Id` member (CS1061 compile errors).

### Implementation for User Story 4

- [X] T039 [US4] Add `Id` to `AuthResponse` (`backend/TaskFlow.Api/Dtos/AuthResponse.cs`) and `MeResponse` (`backend/TaskFlow.Api/Dtos/MeResponse.cs`), update `AuthService.IssueToken` and `AuthController.Me` to populate it (research.md §8); add `id: number` to `AuthState`/the internal API-response interface in `frontend/src/app/auth/auth.service.ts`, and update the existing `AuthState` object literals in `auth.service.spec.ts`, `app.spec.ts`, `shared/header/header.component.spec.ts`, `auth/auth.guard.spec.ts`, and `auth/admin.guard.spec.ts` accordingly — depends on T038d (confirmed RED). Also found and updated a sixth literal in `home/home.component.spec.ts` not listed in the original task text. **Confirmed GREEN**: 82/82 backend tests, 43/43 frontend tests, 0 regressions.
- [X] T040 [US4] Implement `WorkItemService.UpdateAsync` (creator/assignee/role check, full-field replace, advances `UpdatedAt`) in `backend/TaskFlow.Api/Services/WorkItemService.cs` — depends on T031
- [X] T041 [US4] Implement `WorkItemsController.Update` (`PUT /api/work-items/{id}`) and `Get` (`GET /api/work-items/{id}`) in `backend/TaskFlow.Api/Controllers/WorkItemsController.cs` — depends on T040
- [X] T041a [US4] Implement `WorkItemService.DeleteAsync` (creator/Manager/Admin check — narrower than `UpdateAsync`, no assignee-alone case) in `backend/TaskFlow.Api/Services/WorkItemService.cs` — depends on T031
- [X] T041b [US4] Implement `WorkItemsController.Delete` (`DELETE /api/work-items/{id}`, returns `204`) in `backend/TaskFlow.Api/Controllers/WorkItemsController.cs` — depends on T041a. **Confirmed GREEN** (all four backend tasks together, plus a bare-minimum `GetWorkItemsAsync`/`GetWorkItems` — no filters yet — pulled forward per the discovered-dependency note above): 133/133 backend tests pass (was 82; +51 new, 0 regressions).
- [X] T042 [US4] Extend the `work-item-form` component to support edit mode (pre-fill from an existing item) — depends on T037, T033. Added a `projects/:projectId/work-items/:id/edit` route; presence of the `:id` param distinguishes edit from create mode in the same component.
- [X] T043 [US4] Add `getWorkItem()`/`updateWorkItem()` to `work-items.service.ts`; show/hide each item's **edit** control in `project-detail` by comparing `AuthService`'s current user id/role against the item's creator/assignee (creator, current assignee, or Manager/Admin) — depends on T038, T039, T041, T042. Also added a basic (unfiltered) `getWorkItems()` call in `project-detail` to render the rows T038/T038c's controls attach to (the discovered-dependency note above).
- [X] T043a [US4] Add `deleteWorkItem()` to `work-items.service.ts`; show/hide each item's **delete** control in `project-detail` using the narrower creator/Manager/Admin rule (no assignee-alone), and wire a simple confirmation (research.md §5) before calling delete — depends on T038c, T039, T041b. Confirmation uses the browser's native `confirm()`, consistent with research.md §5's "client-side, not a dedicated endpoint" decision. **Confirmed GREEN** (T037/T038/T038c/T042/T043/T043a together): 66/66 frontend tests pass (was 43; +23 new, 0 regressions).

**Checkpoint**: User Stories 1-4 work — the full create/view/edit/delete loop is functional. **Verified end-to-end in a real browser** against the live API/database (Playwright): created a project and work item as Admin, opened the edit page (title/type/priority/status/assignee all correctly pre-filled, including each `<select>`), saved a title change and confirmed it reflected in the list, then deleted the item via the confirmation dialog and confirmed the list returned to "No work items yet." Zero console errors. 133/133 backend tests, 66/66 frontend tests green throughout.
...
- [X] T044 [P] [US5] Unit tests for `ProjectService.UpdateAsync`/`DeleteAsync` in `backend/TaskFlow.Api.Tests/Services/ProjectServiceTests.cs`: edit updates fields and rejects a name that duplicates a *different* project (not itself); delete removes the project and, via cascade, all of its work items. **Confirmed RED**: `ProjectService` had no `UpdateAsync`/`DeleteAsync` (CS1061).
- [X] T045 [P] [US5] Integration tests for `PUT /api/projects/{id}` and `DELETE /api/projects/{id}` in `backend/TaskFlow.Api.Tests/Integration/ProjectsEndpointsTests.cs`: `200`/`204` for Manager/Admin, `403` for a Developer, `404` unknown id, `409` on a duplicate name
- [X] T046 [P] [US5] Vitest tests for edit mode in `project-form.component.spec.ts`, and for `project-detail`'s delete confirmation in `project-detail.component.spec.ts`: confirmation states the exact work-item count from `totalWorkItemCount`; edit/delete controls hidden for a non-Manager/Admin. Also added the equivalent `projects-list` Edit-link visibility tests (T050). **Confirmed RED**: 9 new tests failed (missing pre-fill, missing `.project-edit-link`/`.project-delete-button` elements).

### Implementation for User Story 5

- [X] T047 [US5] Implement `ProjectService.UpdateAsync` and `DeleteAsync` (duplicate-name check excludes the project being edited) in `backend/TaskFlow.Api/Services/ProjectService.cs` — depends on T018
- [X] T048 [US5] Implement `ProjectsController.Update` (`PUT`) and `Delete` (`DELETE`) with `[Authorize(Roles = "Manager,Admin")]` in `backend/TaskFlow.Api/Controllers/ProjectsController.cs` — depends on T047. **Confirmed GREEN**: 125/125 backend tests pass (was 111; +14 new, 0 regressions).
- [X] T049 [US5] Extend `project-form` to support edit mode; add a delete control and confirmation dialog (using the already-fetched `totalWorkItemCount`) to `project-detail` — depends on T046, T012, T021. Added a `projects/:id/edit` route (registered before `projects/:id` in `app.routes.ts`).
- [X] T050 [US5] Add `updateProject()`/`deleteProject()` to `projects.service.ts`; show "Edit"/"Delete" controls in `project-detail` and `projects-list` only when `currentRole()` is Manager or Admin — depends on T049. **Scope note**: `projects-list` gets an Edit link only, not Delete — an accurate "this will delete N items" confirmation needs `totalWorkItemCount`, only returned by `GET /api/projects/{id}` (project detail), not the list endpoint's `openWorkItemCount` (research.md §5); duplicating an inaccurate confirmation on the list page would be a correctness regression, so Delete stays exclusively on `project-detail`. **Confirmed GREEN**: 69/69 frontend tests pass (was 58; +11 new, 0 regressions).

**Checkpoint**: User Stories 1-5 work — full project and work-item lifecycle management. **Verified end-to-end in a real browser** (Playwright): created a project with two work items as Admin, edited the project's name (pre-filled correctly, save reflected in the header), then deleted it — the confirmation dialog stated the exact count ("This will also delete 2 work item(s)"), and after confirming, the app redirected to `/projects` and the deleted project no longer appeared in the list. Zero console errors. 125/125 backend tests, 69/69 frontend tests green throughout.
...
- [X] T058 [P] Add a "Projects" navigation entry to the header, visible to every signed-in user (unlike "Users", which stays Admin-only), in `frontend/src/app/shared/header/`. **Confirmed RED then GREEN**: 77/77 frontend tests pass (was 74; +3 new, 0 regressions).
- [X] T058a [P] Retrofit `project-form`, `projects-list`, `project-detail`, and `work-item-form` (all built pre-styling-pass with plain HTML) to Angular Material components (`mat-card`, `mat-form-field`/`mat-input`, `mat-table`, `mat-button`), matching Feature 001's restyled pages, so the feature doesn't ship in a mixed style — keep every native `<select>` with `[selected]`-per-`<option>` exactly as-is (research.md §6); all existing Vitest tests for these components must stay green, updating only selectors that change because of the template restructuring. Added a new `.form-page` global CSS class (styles.css) — same centered single-form layout as `.auth-page`, but not reusing that class name since these aren't authentication pages, and Feature 001's own templates were left untouched. **Confirmed GREEN**: 77/77 frontend tests pass throughout, zero regressions across all four components. **Verified visually in a real browser** (Playwright, screenshots reviewed): projects-list renders as a `mat-table`, project-form/work-item-form as a centered `mat-card`, project-detail's header as a `mat-card` with its work-item list as a `mat-table` — native `<select>` filters/dropdowns retained throughout. Full create→edit→delete round-trip re-verified against the retrofitted markup, zero console errors.
- [X] T059 [P] Review all new and changed C# files for the constitution's required educational comments — especially the cascade-path rule (research.md §2), combined role-and-ownership authorization, and conditional `IQueryable` filtering (plan.md "Concepts You Will Learn"). Found solid coverage already in place from earlier phases (`AppDbContext.cs`'s cascade-path comment, `WorkItemService`'s authorization comments); added one missing comment explaining `GetWorkItemsAsync`'s conditionally-appended `.Where()` calls building one deferred `IQueryable` rather than executing per-filter. Controllers stay comment-free by design — the mechanical route→service→exception-mapping pattern doesn't need re-explaining at every action.
- [X] T059a [P] Add an integration test verifying every error response (`400`/`403`/`404`/`409`) from the Projects and Work Items endpoints is a valid RFC 7807 `ProblemDetails` body, extending Feature 001's equivalent `ProblemDetails`-shape test. **Real bug found and fixed**: `[Authorize(Roles = ...)]`-driven 401/403 responses (rejected by the authorization middleware itself, never reaching a controller's own `Problem(...)` call) were returning a completely empty body — `AddProblemDetails()` was registered in `Program.cs` but nothing converted framework-level rejections into one. Added `app.UseStatusCodePages()` right after `UseExceptionHandler()` to close the gap; this fixes the same latent issue for Feature 001's `[Authorize(Roles = "Admin")]` endpoints too, not just Feature 002's. Also confirmed data-annotation-driven `400`s are `ValidationProblemDetails` (an `Errors` dict, no top-level `Detail`) versus business-exception `400`s (a `Detail` message, no `Errors`) — both are the documented contract (contracts/*.md: "ProblemDetails / ValidationProblemDetails"), not an inconsistency. **Confirmed GREEN**: 137/137 backend tests pass (was 129; +8 new, 0 regressions).
- [X] T060 Verify `dotnet ef database update` applies this feature's migration cleanly to a fresh database, extending Feature 001's equivalent check. Also ran a throwaway `dotnet ef migrations add` (empty `Up()`/`Down()`) to confirm the model has zero pending changes beyond the committed migration. Verified via a real throwaway database (`TaskFlowDb_MigrationCheck`, created and dropped): both `AddUsersTable` and `AddProjectsAndWorkItems` applied cleanly in order, including the cascade/restrict FK behavior exactly as designed (`WorkItems.ProjectId` → `ON DELETE CASCADE`; both `User`-pointing FKs → `ON DELETE NO ACTION`).
- [X] T061 Add a "What I learned" entry for this feature to the project `README.md`. Covers three concrete lessons: the `AddProblemDetails()`/`UseStatusCodePages()` gap discovered by T059a, the SQL Server multiple-cascade-paths modeling decision (research.md §2), and the US4/US6 task-dependency discovery made and documented during this implementation.
- [X] T062 Run all `quickstart.md` validation scenarios end-to-end manually. **Found and fixed a stale doc**: scenario 4 hadn't been updated for the analyze-triage's US4 delete scope (E1) — it only described editing, not deleting, a work item; updated it to match spec.md's current acceptance scenarios before validating. All 7 scenarios verified end-to-end in a real browser (Playwright) against the live API/database: duplicate-name rejection (case-insensitive) and Developer-blocked-from-creating (S1); paginated list + empty-project messaging (S2); default priority/status + past-due-date/assignee acceptance (S3); edit reflected live, unrelated-viewer sees no controls, direct API update refused 403, delete removes the item (S4); delete confirmation states the exact item count, Developer blocked from project edit/delete (S5); priority filter and no-match search behave correctly (S6); and the full combined success check, including a direct API project-create attempt by a Developer refused with 403. Zero console errors throughout.
- [X] T063 Confirm zero build warnings (`dotnet build`) and all tests pass (`dotnet test`, and the frontend Vitest suite). `dotnet build` on both `TaskFlow.Api` and `TaskFlow.Api.Tests` shows zero compiler warnings — only the pre-existing `NU1903` NuGet advisory for `Microsoft.OpenApi` (present since Feature 001's setup, unrelated to this feature). **Final confirmed GREEN**: 137/137 backend tests, 77/77 frontend tests.
