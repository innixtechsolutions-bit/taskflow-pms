...
- [X] T024a *(bug fix, found in manual browser testing after the checkpoint below)* Wire up frontend routing, which had never actually been connected: `app.routes.ts` still held the scaffolded `routes: Routes = []`, and `app.html` still rendered the full `ng new` placeholder markup unconditionally above `<router-outlet />` — so T023/T024's tests passed (they render `RegisterComponent` directly via `TestBed`, bypassing the router) while the real browser at `/register` showed the Angular starter page regardless of URL. Fixed by adding `{ path: 'register', component: RegisterComponent }` and a `'' → '/register'` redirect (no wildcard yet — deferred until US2 adds more real destinations, so a catch-all can't swallow future route typos silently) to `app.routes.ts`, and reducing `app.html` to just `<router-outlet />` (`app.ts`'s now-unused `title` signal removed to match). Also found and fixed two adjacent gaps surfaced by the same manual test: (1) no dev-server proxy existed, so `AuthService`'s relative `/api/auth/register` call would have 404'd against the Angular dev server itself instead of reaching the backend — added `frontend/proxy.conf.json` (`/api` → `http://localhost:5146`) and wired it via `angular.json`'s `serve.options.proxyConfig`; (2) the backend had no CORS policy at all — added a `FrontendDev` policy (`http://localhost:4300`, no credentials since the JWT travels in an `Authorization` header, not a cookie) in `Program.cs`, applied via `app.UseCors(...)` before `UseAuthentication`/`UseAuthorization` (CORS must run before auth, since the browser's preflight `OPTIONS` carries no Authorization header). Port 4300 pinned permanently via `angular.json`'s `serve.options.port` (previously unset, defaulting to the CLI's 4200). `app.spec.ts` rewritten to assert against real routing (`RouterTestingHarness`) instead of the placeholder's `h1` text. **Confirmed GREEN**: 15/15 backend tests, 10/10 frontend tests. **Verified in a real browser** via a headless-Chromium script (not just test doubles): `http://localhost:4300/` redirects to `/register`, the real form renders (no placeholder text present), and a live submission against the real API/database returns `201` with zero console errors.
...
- [X] T025 [P] [US2] Unit tests for `AuthService.LoginAsync` and `LoginAttemptTracker` in `backend/TaskFlow.Api.Tests/Services/AuthServiceTests.cs` and `backend/TaskFlow.Api.Tests/Services/LoginAttemptTrackerTests.cs`: same generic error for wrong email vs. wrong password, 8h token expiry set correctly, 6th attempt within 15 minutes blocked, counter clears on success. **Confirmed RED**: compile errors (`ILoginAttemptTracker`/`LoginRequest`/`LoginAttemptTracker`/`AuthService.LoginAsync` didn't exist).
- [X] T026 [P] [US2] Integration tests for `POST /api/auth/login`, `POST /api/auth/logout`, `GET /api/auth/me` in `backend/TaskFlow.Api.Tests/Integration/AuthEndpointsTests.cs`: `200` on success, `401` generic message for bad credentials, `429` after rate limit, `me` returns `401` for missing/expired token and `200` for a valid one (allowed + denied paths). Expired-token case hand-builds a JWT signed with the test host's own signing key but an `exp` in the past, rather than waiting out a real 8-hour window.
- [X] T027 [US2] Integration test asserting the `ProblemDetails` response shape (`type`/`title`/`status`/`detail` present and populated) on a representative error response — the `401` from `POST /api/auth/login` with bad credentials — in `backend/TaskFlow.Api.Tests/Integration/AuthEndpointsTests.cs` (same file as T026).
- [X] T028 [P] [US2] Vitest tests for the login component and route guard in `frontend/src/app/auth/login/login.component.spec.ts` and `frontend/src/app/auth/auth.guard.spec.ts`: generic error message rendering; guard redirects unauthenticated access to `/login?returnUrl=...` and returns there post-login; **and** an expired-token scenario asserts the guard redirects to `/login` with a distinct `expired=true` query flag (FR-010) rather than the plain "not logged in" `returnUrl`-only redirect. **Confirmed RED**: `Cannot find module './login.component'` / `'./auth.guard'` build errors. Also added `auth.interceptor.spec.ts` (no prior test existed for this file at all) covering the new expired-session redirect added in T035, and a small test-first `home.component.spec.ts` for the placeholder `HomeComponent` added below (see T033a note).

### Implementation for User Story 2

- [X] T029 [US2] Implement `LoginAttemptTracker` (`IMemoryCache`-backed, per-email 15-minute window) in `backend/TaskFlow.Api/Services/LoginAttemptTracker.cs`. Registered `IMemoryCache` + `ILoginAttemptTracker` as Singleton in `Program.cs` — Singleton, not Scoped, because it must accumulate failures across requests.
- [X] T030 [US2] Create `LoginRequest` DTO in `backend/TaskFlow.Api/Dtos/LoginRequest.cs`. Also added `MeResponse` (`Dtos/MeResponse.cs`, not separately tasked — `GET /me` needed a response shape distinct from `AuthResponse` since it has no token to return) and `InvalidCredentialsException`/`TooManyAttemptsException` in `Services/AuthExceptions.cs`.
- [X] T031 [US2] Implement `AuthService.LoginAsync` (rate-limit check via T029, generic invalid-credentials error, JWT issuance) in `backend/TaskFlow.Api/Services/AuthService.cs` — depends on T029, T030. **Confirmed GREEN**: all new `AuthServiceTests`/`LoginAttemptTrackerTests` pass.
- [X] T032 [US2] Implement `AuthController.Login`, `Logout`, `Me` actions in `backend/TaskFlow.Api/Controllers/AuthController.cs` — depends on T031. `Logout` is `[Authorize]` + `204 NoContent` with no server-side effect beyond the auth check itself (stateless JWTs have no session to end — see `research.md` §6). **Bug found and fixed while getting T026/T027 GREEN**: `Program.cs`'s JWT bearer setup read `Jwt:SigningKey`/`Issuer`/`Audience` into local variables *before* `builder.Build()`, then captured them in the `AddJwtBearer` options closure. Against `TaskFlowApiFactory` (which layers in a different signing key via `ConfigureWebHost` partway through host startup), that closure captured the value too early — tokens were issued (by `AuthService`, a live `IConfiguration` read per request) with the *test* key but validated against whatever `Jwt:SigningKey` resolved to at that earlier snapshot point, failing every protected-endpoint call with `WWW-Authenticate: ... error_description="The signature key was not found"`. This is a real bug, not a test-only artifact — any future scenario where `Jwt:SigningKey` becomes available after that snapshot point (a config-reload, a different DI-composition order) would have broken production auth the same way. Fixed by moving the actual `TokenValidationParameters` construction inside the `AddJwtBearer` lambda, reading `builder.Configuration` fresh there (the options framework defers this delegate until the handler is first resolved, i.e. after all config layering is complete) — the up-front reads stay, now discarded, purely to fail fast on a real misconfigured deployment. **Confirmed GREEN**: all 35 backend tests pass (was 15 before US2; +20 new, 0 regressions) with 0 new build warnings.
- [X] T033 [US2] Build the Angular login page (`frontend/src/app/auth/login/login.component.ts` + template) with inline validation and the generic error message — depends on T015. Also added `AuthService.login()`/`logout()`.
- [X] T033a *(small addition, not separately tasked — surfaced by a scope question raised and resolved with the maintainer)* Added a minimal placeholder `HomeComponent` (`frontend/src/app/home/`) — no user story in this feature ever tasked building actual home-page content, but `authGuard` needs a real protected route to guard, and register/login both navigate to `/` on success. Shows just the signed-in person's name and role via `AuthService`; will be replaced by real content in a later feature.
- [X] T034 [US2] Implement `frontend/src/app/auth/auth.guard.ts` (functional `CanActivateFn`, redirect to `/login?returnUrl=`) and apply it to protected routes in `frontend/src/app/app.routes.ts` — depends on T015. `app.routes.ts` now has three routes: `'' ` (`HomeComponent`, guarded), `login`, `register` — the guard distinguishes "never signed in" (`returnUrl` only) from "signed in but the token's `expiresAt` has passed" (clears state, adds `expired=true`) since only the latter should show the FR-010 message.
- [X] T035 [US2] Wire session-expiry handling: interceptor/guard react to an expired token by clearing `AuthService` state and redirecting to login with "Your session has expired.", and add the logout call/state-clear to `AuthService` — depends on T015, T016, T032. The interceptor only treats a `401` as "session expired" (clear + redirect) when `AuthService.isAuthenticated()` was already `true` going in — otherwise a plain wrong-password `401` from the login form itself would trigger the same redirect and yank the visitor away before `LoginComponent` ever shows its own error message. **Confirmed GREEN**: 22/22 frontend tests pass (was 10 before US2; +12 new, 0 regressions).
...
- [X] T039 [P] [US4] Unit tests for `UserService` in `backend/TaskFlow.Api.Tests/Services/UserServiceTests.cs`: pagination shape, role change takes effect, last-remaining-Admin self-demotion is rejected. Also covers the corollary the spec calls out explicitly (spec.md Edge Cases): an Admin may always demote a *different* Admin, even the only other one, since the caller remains an Admin afterward either way. **Confirmed RED**: compile error (`UserService` didn't exist).
- [X] T040 [P] [US4] Integration tests for `GET /api/users` and `PUT /api/users/{id}/role` in `backend/TaskFlow.Api.Tests/Integration/UsersEndpointsTests.cs`: `200` + paginated list and successful role change for an Admin caller, `403` for a non-Admin caller on both endpoints, `400` for the last-Admin guard, `404` for an unknown user id (allowed + denied paths). Admin calls authenticate as `TaskFlowApiFactory`'s seeded Admin (`admin@taskflow.local`) via a real `POST /api/auth/login` rather than a fixture shortcut.
- [X] T041 [P] [US4] Vitest tests for the users-list component in `frontend/src/app/users/users-list/users-list.component.spec.ts`: renders the paginated list, triggers a role change. **Confirmed RED**: `Cannot find module './users-list.component'` build error. (The third behavior in this task's description — "is not reachable/rendered for a non-Admin" — is a routing/navigation concern, not something the list component itself controls; covered instead by `admin.guard.spec.ts` and two new `header.component.spec.ts` cases under T047, which is the layer that actually owns that decision.)

### Implementation for User Story 4

- [X] T042 [US4] Create `UserListItemDto` and `ChangeRoleRequest` DTOs in `backend/TaskFlow.Api/Dtos/`. Also added `UserNotFoundException`/`InvalidRoleException`/`LastAdminException` in `Services/UserExceptions.cs` (not separately tasked, same pattern as `AuthExceptions.cs`).
- [X] T043 [US4] Implement `UserService` (paginated list, role change with the last-Admin guard evaluated immediately before commit) in `backend/TaskFlow.Api/Services/UserService.cs` — depends on T006, T007, T042. Registered as Scoped in `Program.cs`. **Confirmed GREEN**: all new `UserServiceTests` pass.
- [X] T044 [US4] Implement `UsersController` (`GET /api/users`, `PUT /api/users/{id}/role`) with `[Authorize(Roles = "Admin")]` on both actions in `backend/TaskFlow.Api/Controllers/UsersController.cs` — depends on T043. **Confirmed GREEN**: 48/48 backend tests pass (was 35; +13 new, 0 regressions).
- [X] T045 [US4] Build `frontend/src/app/users/users.service.ts` calling the Users API — depends on T016.
- [X] T046 [US4] Build the `users-list` component (paginated table, role-change control) in `frontend/src/app/users/users-list/` (+ template) — depends on T045. **Confirmed GREEN**: both new tests pass.
- [X] T047 [US4] Add an Admin-only route guard (extend/compose with `auth.guard.ts`) for the Users route in `frontend/src/app/app.routes.ts`, and add the "Users" navigation entry only for Admins in the header — depends on T034, T037, T046. New `admin.guard.ts` composes `authGuard` (same session/expiry handling, no duplicated logic) then additionally checks `currentRole() === 'Admin'`, redirecting a signed-in non-Admin to `/` (not `/login` — they're already authenticated, just not authorized). Header gets a conditional "Users" link, test-first (`header.component.spec.ts` — **confirmed RED**, `TypeError: currentRole is not a function`, then GREEN). 33/33 frontend tests pass (was 26; +7 new, 0 regressions).

**Checkpoint**: All four user stories are independently functional — the feature described in `spec.md` is complete end-to-end. **Verified end-to-end in a real browser** against the live API/database: a registered (non-Admin) visitor sees no "Users" link and is redirected to `/` on a direct `/users` visit; signing in as the seeded Admin shows the "Users" link, the Users page lists every real registered person (including ones created by earlier manual-testing sessions), and changing a row's role from the dropdown persists immediately (verified by reading the select's value back after the change completed). Zero console errors.
...
- [X] T048 [P] Verify `dotnet ef database update` applies cleanly to a fresh database and the app fails to start (per T012) when `Admin:Password` is unset — validates quickstart.md scenario 7. Applied migrations to a genuinely new, never-before-seen `TaskFlowDb_FreshCheck_*` database via `dotnet ef database update --connection "..."` (clean, no errors), then ran the app against that same Admin-less database with `Admin__Password=""` — it threw `InvalidOperationException` with the exact configured message and never started listening, confirming FR-018a. Temporary database dropped afterward.
- [X] T049 [P] Password-non-exposure audit (validates SC-005 and constitution Principle II directly, not just incidentally): integration test(s) in `backend/TaskFlow.Api.Tests/Integration/` asserting the response bodies of `POST /api/auth/register`, `POST /api/auth/login`, `GET /api/auth/me`, and `GET /api/users` never contain a `password` or `passwordHash` property (serialize each response and assert the property is absent, not merely null); plus a manual review confirming no logging statement in `AuthService`, `UserService`, or `AdminSeeder` includes a raw password or password hash. New `PasswordExposureTests.cs` recursively walks every response's JSON tree asserting no property name contains "password" (case-insensitive) — **4/4 pass**, confirming the existing DTOs (`AuthResponse`, `MeResponse`, `UserListItemDto`) already got this right by construction. Manual log review: `AdminSeeder`'s two `LogInformation` calls log only "already exists" and the Admin's email, never a password or hash; `AuthService`/`UserService` have no logging at all.
- [X] T050 [P] Review all new C# files for the constitution's required educational comments (DI lifetimes, async/await, DbContext/change tracking, middleware order, DTOs vs. entities — see plan.md "Concepts You Will Learn"). Reviewed all 17 non-migration files in `backend/TaskFlow.Api/`; comment coverage is consistent with the "explain a concept the first time it appears, don't repeat it" rule already established since Foundational (e.g. `LoginRequest`/`UserListItemDto` don't re-explain "DTOs vs. entities" since `RegisterRequest`/`AuthResponse` already did; `UserService`'s Scoped lifetime isn't re-explained since `AuthService`'s registration comment already covers it). No gaps found.
- [X] T051 Add a "What I learned" entry for this feature to the project `README.md` (constitution Definition of Done item 5) — create `README.md` if it does not yet exist. Created `README.md` with a project overview and a "What I learned" entry for this feature covering DI lifetimes, async/await, and — the most concrete lesson of this feature — the closure-timing JWT signing-key bug from T032 (a value captured in a closure is frozen at *definition* time, not *run* time, and those can be further apart than they look across a host-startup boundary).
- [X] T052 Run all `quickstart.md` validation scenarios end-to-end manually. All 7 scenarios re-verified in a real browser against the current committed state (scenario 7 via T048 above; 1–6 via headless Chromium against the live API/database). **This manual pass caught two real bugs in the Users page**, both fixed test-first before T052 could be marked done:
  1. After a *rejected* role change (e.g. the last-Admin guard), the `<select>` kept showing the rejected value instead of the real one — Angular only re-pushes a property binding when the bound expression changes, and a rejected request never changes `user.role`, so the dropdown silently lied about the actual role until the next full reload. Fixed by explicitly resetting `select.value = user.role` in the failure path (`users-list.component.ts`).
  2. On a **fresh page load**, `[value]="user.role"` on the `<select>` itself never correctly pre-selected a non-default role at all (only "Developer," the first `<option>`, ever showed as selected — an Admin's own row showed "Developer" instead of "Admin," and any Manager showed as "Developer" too) — a well-known Angular/native-`<select>` timing issue: Angular writes a `<select>`'s own `[value]` binding before its `@for`-rendered `<option>` children exist in the DOM, so the browser has nothing yet to match against. Fixed by moving the binding to `[selected]` on each individual `<option>` instead. This is the more serious of the two — it would have shown every Admin reviewing the Users page the *wrong role* for anyone who wasn't a plain Developer, on every single page load.

  Both caught by a new `users-list.component.spec.ts` case each (confirmed RED against the pre-fix component, then GREEN) before being declared fixed — not just eyeballed in the browser. 35/35 frontend tests pass (was 33; +2 new).
- [X] T053 Confirm zero build warnings (`dotnet build`) and all tests pass (`dotnet test`, and the frontend Vitest suite) — this includes T049's password-non-exposure audit. `dotnet build`: 0 new warnings (only the pre-existing, documented `NU1903`). `dotnet test`: **52/52** backend tests. Frontend Vitest: **35/35**. Feature 001-user-auth is DONE per the constitution's Definition of Done.
