...
- [X] T004 [P] [US1] Unit tests for `WorkItemService.CreateAsync`'s parent validation in `backend/TaskFlow.Api.Tests/Services/WorkItemServiceTests.cs`: Epic rejects any `ParentWorkItemId`; Story requires an Epic parent; Task's parent is optional but must be a Story when set; SubTask requires a Task parent; parent from a different project is rejected; unknown `ParentWorkItemId` is rejected. **Confirmed RED**: compile errors (`ParentWorkItemId`, hierarchy exceptions didn't exist).
- [X] T005 [P] [US1] Unit tests for `WorkItemService.GetParentCandidatesAsync` in `backend/TaskFlow.Api.Tests/Services/WorkItemServiceTests.cs`: returns only same-project items of the required parent type for the given `type`; returns an empty list for `type=Epic`
...
- [X] T007 [P] [US1] Integration tests for `GET /api/projects/{projectId}/work-items/parent-candidates?type=` in `backend/TaskFlow.Api.Tests/Integration/WorkItemsEndpointsTests.cs`: `200` with correct candidates per type, `200` empty array for `type=Epic`, `400` invalid/missing `type`, `404` unknown project, `401` with no token. **Confirmed RED**: `GetParentCandidatesAsync`/`WorkItemParentCandidatesResponse`/the endpoint didn't exist.
- [X] T008 [P] [US1] Vitest tests for the parent picker (create mode) in `frontend/src/app/projects/work-item-form/work-item-form.component.spec.ts`: picker's candidate list refetches and changes when `Type` changes, is required for `SubTask`, and is disabled/hidden for `Epic`. **Confirmed RED**: 2/3 new tests failed (no `getParentCandidates` call, no `#parentWorkItemId` element yet).

### Implementation for User Story 1

- [X] T009 [P] [US1] Add `WorkItemLookupItemDto` in `backend/TaskFlow.Api/Dtos/WorkItemLookupItemDto.cs` (Id, Title) per `data-model.md`
- [X] T010 [P] [US1] Add `ParentWorkItemId` (`int?`) to `WorkItemRequest` in `backend/TaskFlow.Api/Dtos/WorkItemRequest.cs`
- [X] T011 [P] [US1] Add `ParentWorkItemId` (`int?`) to `WorkItemDto` in `backend/TaskFlow.Api/Dtos/WorkItemDto.cs`
- [X] T012 [US1] Add hierarchy exceptions (`EpicCannotHaveParentException`, `ParentRequiredException`, `InvalidParentTypeException`, `ParentWorkItemNotFoundException`, `ParentMustBeSameProjectException`) in `backend/TaskFlow.Api/Services/WorkItemExceptions.cs` — depends on T009-T011
- [X] T013 [US1] Implement a `RequiredParentType(WorkItemType)` helper and a shared parent-validation routine in `backend/TaskFlow.Api/Services/WorkItemService.cs` (data-model.md's Hierarchy rules table); wire it into `CreateAsync` — depends on T012
- [X] T014 [US1] Implement `WorkItemService.GetParentCandidatesAsync` in `backend/TaskFlow.Api/Services/WorkItemService.cs` (project-scoped, type-filtered per the Hierarchy rules table — research.md §2, no exclude-self/descendants logic needed) — depends on T009
- [X] T015 [US1] Implement `WorkItemsController.GetParentCandidates` (`GET /api/projects/{projectId}/work-items/parent-candidates`) and extend `Create`'s catch blocks for the new exceptions in `backend/TaskFlow.Api/Controllers/WorkItemsController.cs` — depends on T013, T014. **Confirmed GREEN**: 160/160 backend tests pass (was 137, +23 new, 0 regressions).
- [X] T016 [US1] Add `getParentCandidates()` to `frontend/src/app/projects/work-items.service.ts`; add `parentWorkItemId` to the `WorkItem`/`WorkItemRequest` interfaces — depends on T015. Also updated the `WorkItem`-shaped mock literals in `project-detail.component.spec.ts` and `work-item-form.component.spec.ts` (new required field).
- [X] T017 [US1] Extend the `work-item-form` component (create mode) with a parent picker that refetches candidates whenever `Type` changes, using native `<select>` with `[selected]`-per-`<option>` (Feature 002's research.md §6 convention) — depends on T016, T008. **Confirmed GREEN**: 80/80 frontend tests pass (was 77, +3 new, 0 regressions).
...
- [X] T026 [P] [US3] Unit tests for the detail projection in `backend/TaskFlow.Api.Tests/Services/WorkItemServiceTests.cs`: `parentWorkItemId`/`parentTitle` null when no parent, populated when one exists; `children` lists direct children only (title/type/status/assigneeName); `totalDescendantCount` sums every level, not just direct children. **Confirmed RED**: compile errors (`WorkItemDto` had no `ParentTitle`/`Children`/`TotalDescendantCount`).
- [X] T027 [P] [US3] Unit tests for `WorkItemService.DeleteAsync`'s cascade behavior in `backend/TaskFlow.Api.Tests/Services/WorkItemServiceTests.cs`: deleting an item with descendants removes the entire subtree in one call; siblings and ancestors are untouched; the authorization check (creator/Manager/Admin) applies only to the item being deleted, not to each descendant
- [X] T028 [P] [US3] Integration tests for `GET /api/work-items/{id}`'s enriched response in `backend/TaskFlow.Api.Tests/Integration/WorkItemsEndpointsTests.cs`: response includes `parentWorkItemId`/`parentTitle`/`children`/`totalDescendantCount` as specified in `contracts/work-item-hierarchy-api.md`. **Confirmed RED**: `WorkItemDetailDto` didn't exist.
- [X] T029 [P] [US3] Integration tests for `DELETE /api/work-items/{id}`'s cascade behavior in `backend/TaskFlow.Api.Tests/Integration/WorkItemsEndpointsTests.cs`: deleting a parent with descendants returns `204` and removes the whole subtree; `403`/`404` unchanged from Feature 002
- [X] T030 [P] [US3] Vitest tests for a new `work-item-detail` component in `frontend/src/app/projects/work-item-detail/work-item-detail.component.spec.ts`: renders the parent as a link when present and nothing when absent; lists children (title/type/status/assignee) each linking to that child's detail page; starting a new child pre-selects this item as parent; the delete confirmation states `totalDescendantCount` when greater than zero. **Confirmed RED**: `Cannot find module './work-item-detail.component'`.

### Implementation for User Story 3

- [X] T031 [P] [US3] Add `WorkItemChildDto` (`backend/TaskFlow.Api/Dtos/WorkItemChildDto.cs`) and `WorkItemDetailDto` (`backend/TaskFlow.Api/Dtos/WorkItemDetailDto.cs`) per `data-model.md`
- [X] T032 [US3] Implement a recursive descendant-id collection helper in `backend/TaskFlow.Api/Services/WorkItemService.cs`, shared by the detail projection's `totalDescendantCount` and `DeleteAsync`'s cascade (research.md §1, §6) — depends on T031
- [X] T033 [US3] Change `WorkItemService.GetByIdAsync` to return `WorkItemDetailDto` via a new `ToDetailDtoAsync` (parent lookup, direct-children projection, `totalDescendantCount`) — depends on T032
- [X] T034 [US3] Update `WorkItemService.DeleteAsync` to collect all descendant ids via the T032 helper and `RemoveRange` the entire subtree in one `SaveChangesAsync` call — depends on T032. **Confirmed GREEN**: 176/176 backend tests pass (was 168, +8 new, 0 regressions).
- [X] T035 [US3] Update `WorkItemsController.Get`'s return type to `WorkItemDetailDto` (no route change) — depends on T033
- [X] T036 [US3] Build the `work-item-detail` component + template in `frontend/src/app/projects/work-item-detail/`, reusing `project-detail`'s existing edit/delete permission logic — depends on T030, T035
- [X] T036a [US3] Wire the `work-item-detail` component's "create child" action (FR-019): a link/button, shown only when this item's type can legally have children (Epic, Story, or Task), that navigates to work-item creation with this item pre-selected as parent — e.g. a `parentWorkItemId` route/query param that `work-item-form`'s create mode reads and pre-fills (reusing T017's parent-picker population, just with an initial value) — depends on T036, T017. Also passes an inferred `type` query param (the correct immediate child type) so the created item defaults to a legal type, not just a legal parent.
- [X] T037 [US3] Add `getWorkItemDetail()` to `work-items.service.ts`; add the `projects/:projectId/work-items/:id` route in `frontend/src/app/app.routes.ts` (registered after the existing `.../work-items/new` and `.../work-items/:id/edit` routes); link work item titles (in both the flat list and the tree view) to the new detail page; update delete confirmations project-wide to show `totalDescendantCount` when greater than zero — depends on T034, T036. The flat list's existing delete button now fetches the item's detail (for `totalDescendantCount`) at the moment delete is clicked, rather than carrying that count on every row (research.md §6). **Confirmed GREEN**: 94/94 frontend tests pass (was 85, +9 new, 0 regressions).
...
- [X] T038 [P] [US4] Unit tests for `WorkItemService.UpdateAsync`'s parent reassignment in `backend/TaskFlow.Api.Tests/Services/WorkItemServiceTests.cs`: valid reparent within the same project succeeds; the same rule violations as `CreateAsync` (T004) are rejected; clearing an optional Task parent succeeds; attempting to set an item as its own parent (self-reference) is rejected — caught by the same type check as every other violation (research.md §2), not a special-cased algorithm
- [X] T039 [P] [US4] Unit tests for `WorkItemService.UpdateAsync`'s type-change guard in `backend/TaskFlow.Api.Tests/Services/WorkItemServiceTests.cs`: refuses a type change when the item's existing parent's type no longer matches the new type's required parent type; refuses a type change when any existing child's required-parent type no longer matches the new type; allows a type change with no such conflict. **Bug found and fixed during GREEN verification**: the children-guard test's original setup (a Task whose *own* parent was a Story) also tripped the parent-guard the instant the type changed away from Task, so the wrong exception fired — rewritten to use a standalone (parentless) Task so only the children-side check is exercised.
...
- [X] T041 [P] [US4] Vitest tests for edit mode's parent picker in `work-item-form.component.spec.ts`: pre-fills the item's current parent; changing/clearing it submits correctly

### Implementation for User Story 4

- [X] T042 [US4] Extend `WorkItemService.UpdateAsync` to reuse US1's parent-validation routine (T013) for the incoming `ParentWorkItemId` — depends on T013
- [X] T043 [US4] Add the type-change guard to `WorkItemService.UpdateAsync` (data-model.md's Hierarchy rules table checked against the item's *current* parent and children — research.md §3), plus `TypeChangeInvalidatesParentException`/`TypeChangeInvalidatesChildrenException` in `WorkItemExceptions.cs` — depends on T042
- [X] T044 [US4] Extend `WorkItemsController.Update`'s catch blocks for the new exceptions in `backend/TaskFlow.Api/Controllers/WorkItemsController.cs` — depends on T043. **Confirmed GREEN**: 187/187 backend tests pass (was 176, +11 new, 0 regressions). One transient run reported 31 failures under parallel SQL-Server-backed test execution (each test class provisions/drops its own database — research.md/SqlServerTestDatabase); a clean immediate re-run reproduced only the one real bug above, confirming the rest was infrastructure flakiness, not a regression.
- [X] T045 [US4] Extend `work-item-form` edit mode: pre-fill the current parent, refresh candidates on `Type` change (reusing T017's logic), submit `parentWorkItemId` — depends on T044, T041. Already substantively implemented alongside T017 (the parent picker was built to serve both create and edit modes from the start); this task's remaining scope was the T041 tests themselves. **Confirmed GREEN**: 97/97 frontend tests pass (was 94, +3 new, 0 regressions).
...
- [X] T049 [P] Review all new/changed C# files for the constitution's required educational comments — especially the self-referencing-cascade restriction (research.md §1), the rank-argument cycle proof (research.md §2), and validating a change against existing state rather than just the incoming request (research.md §3) — per plan.md's "Concepts You Will Learn". **Found solid coverage already in place** from each story's own implementation pass (`AppDbContext.cs`/`WorkItem.cs`'s cascade-restriction comments, `RequiredParentType`'s rank-proof comment, `UpdateAsync`'s type-change-guard comment, `WorkItemTreeNodeDto.cs`/`GetTreeAsync`'s recursive-DTO comments) — no gaps needed filling. Controllers stay comment-free by design, matching Feature 002's precedent.
- [X] T050 [P] Extend Feature 002's `ProblemDetails`-shape regression test to cover this feature's new `400` error cases (hierarchy rule violations, type-change guard) — added to `ProblemDetailsShapeTests.cs`. **Confirmed GREEN**: 10/10 tests in that file pass.
- [X] T051 Verify `dotnet ef database update` applies `AddWorkItemHierarchy` cleanly to a fresh database; confirm zero pending model changes via a throwaway `dotnet ef migrations add`. **Confirmed**: a throwaway `dotnet ef migrations add` produced empty `Up()`/`Down()` methods (zero pending changes); applying all three migrations (`AddUsersTable`, `AddProjectsAndWorkItems`, `AddWorkItemHierarchy`) to a brand-new `TaskFlowDb_MigrationCheck_003` database succeeded cleanly in order, including the self-referencing FK's `ON DELETE NO ACTION`. Throwaway database dropped afterward.
- [X] T052 Add a "What I learned" entry for this feature to the project `README.md`. Covers three lessons: the self-referencing-FK cascade restriction (a different root cause than Feature 002's multiple-cascade-paths case), the rank-argument proof that eliminated an entire cycle-detection algorithm before it was ever written, and a real test-design bug found during US4's GREEN verification (plus the transient SQL-Server-parallel-test flakiness discovered alongside it).
- [X] T053 Run all `quickstart.md` validation scenarios end-to-end manually (REST client + real browser). No browser-automation tool was available this session, so browser-only scenarios (visual tree rendering, expand/collapse clicks, the detail page's "Add child" link) are **not visually verified** — they rely on the Vitest coverage from each story instead, which does exercise the same DOM/behavior assertions. What *was* verified live, against the running API and the real dev SQL Server database (not just the xUnit test harness): built a full Epic→Story→Task→SubTask chain plus a standalone item via real HTTP calls; confirmed the tree endpoint nests them correctly with the standalone item at top level; confirmed all three invalid-parent cases (wrong type, Epic-with-parent, missing-required-parent) return `400`; confirmed a direct self-parent attempt is refused by the ordinary type check; confirmed the Story's detail response shows its parent link, its one direct child, and `totalDescendantCount: 2`; deleted the Story and confirmed both the Task and SubTask beneath it were gone in one operation while the Epic remained; confirmed the flat list's search and unfiltered count are correct afterward. All throwaway data was cleaned up (project deleted via cascade, temporary API process stopped).
- [X] T054 Confirm zero build warnings (`dotnet build`) and all backend/frontend tests pass. **Confirmed**: `dotnet build` on `TaskFlow.Api` shows zero compiler warnings (only the pre-existing `NU1903` `Microsoft.OpenApi` advisory, present since Feature 001, unrelated to this feature). **Final GREEN**: 191/191 backend tests, 98/98 frontend tests. **Discovered, out of scope**: `ng build` (production bundle) fails on a pre-existing gap — `@angular/animations` was never added as an explicit dependency even though Feature 002's Material styling pass introduced `provideAnimationsAsync()`; confirmed via `package.json`'s git history that this predates this feature, and Feature 002's own Definition-of-Done check (`T063`) never ran `ng build`, only `dotnet build`/`dotnet test`/the Vitest suite. Flagging for a future fix rather than resolving here, since it's unrelated to work item hierarchy.
