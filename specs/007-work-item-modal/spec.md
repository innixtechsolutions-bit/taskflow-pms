...
- **FR-005**: On successful create or save, the system MUST close the modal (except when "Create another" is checked during create — see FR-012), show a confirmation toast, and refresh the underlying view in place without a full page reload, preserving board scroll position, expanded tree nodes, and active filters.
...
- FR-015's "detail" surface for label chips means the work item's own labels on its own detail view, not the labels of items in that detail page's compact children sub-list (which today already omits other fields like Priority and Due date for the same reason — it's a minimal row, not a full item view).
